...
def run():
    ...
    x = puq.NormalParameter('x', 'x', mean=5, dev=0.2, data=exp_x)
    y = puq.NormalParameter('y', 'y', mean=3.4, dev=0.25, data=exp_y)
    ...
    return puq.Sweep(uq, host, prog, data=exp_data, err=sigma)
